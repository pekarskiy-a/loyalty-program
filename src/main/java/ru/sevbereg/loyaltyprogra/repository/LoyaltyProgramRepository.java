...
public interface LoyaltyProgramRepository extends JpaRepository<LoyaltyProgram, Long> {
    ...
    @Query(
            value = "SELECT loyaltyProgram FROM LoyaltyProgram loyaltyProgram LEFT JOIN FETCH loyaltyProgram.tiers"
    )
    List<LoyaltyProgram> findAll();

    @Query(
            value = "SELECT loyaltyProgram FROM LoyaltyProgram loyaltyProgram LEFT JOIN FETCH loyaltyProgram.tiers WHERE loyaltyProgram.id = ?1"
    )
